...
client.loadCommands();
```
**The two snippets above is all there is to get your first command working.**  
Alternatively, you can dynamically import the files in which your interactions are defined.  
For example, the code below recursively imports all files inside the folder `/interactions/`. Therefore, for creating new interactions, all you have to do is create the interaction class, decorate it with `@Register...` and rebuild and restart the application
...
// This function recursively gets all files that are in the /interactions/ folder and imports them
